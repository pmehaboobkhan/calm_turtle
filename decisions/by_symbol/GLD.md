...
- Reconciles carried-over W27 prediction (`reports/learning/weekly_learning_review_2026-07-04.md` §2, item 4): "GLD reclaims its 210d MA and unblocks the overlay" (low confidence; no near-term catalyst identified).
- **Outcome: overlay stayed BLOCKED all of W28; the 210d-MA reclaim itself is UNVERIFIABLE on stale data.** The only GLD decision this week — `decisions/2026-07-07/1649_GLD.json` (NO_TRADE) — fired because the deterministic regime engine returned uncertain/low with ALL indicators null (today's daily bars failed to load: yfinance TLS-blocked; Alpaca IEX daily bars stale). With indicators null the 210d MA is not computable, so whether GLD reclaimed it cannot be confirmed either way. Observable outcome: the standing gold_permanent_overlay-ENTRY vs dual_momentum_taa-EXIT conflict (since ~06-11) persisted; GLD stayed FLAT and the overlay remained un-executable — consistent with the low-confidence-of-reclaim base case. Sources: `decisions/by_symbol/GLD.md` (2026-07-07 block), `memory/prediction_reviews/2026-07-07.md`.
- Note: no GLD trade in W28. This block records the prediction outcome only.

## 2026-07-19 — weekly_review self_learning: prediction reconciliation (W29)
- Carries the W27/W28 open item: "GLD reclaims its 210d MA and unblocks the overlay" (low confidence; no near-term catalyst).
- **Outcome: overlay stayed BLOCKED all of W29; the 210d-MA reclaim itself remains UNVERIFIABLE on stale data.** The recurring `gold_permanent_overlay`(ENTRY) vs `dual_momentum_taa`(EXIT) GLD conflict logged again every session (e.g. `logs/risk_events/2026-07-16_204500_signal_conflict.md`; `journals/daily/2026-07-17.md` pre-market signals show GLD below 210d MA → TAA EXIT). GLD not held; Strategy C stayed at 0% vs 10% target. On stale/failed daily bars the 210d MA is not reliably computable, so the reclaim cannot be confirmed either way — consistent with the low-confidence-of-reclaim base case. Carried forward to the next fresh close. No GLD trade in W29.
