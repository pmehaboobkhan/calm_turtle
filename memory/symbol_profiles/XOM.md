...
- Strategy: large_cap_momentum_top5. Position (BUY 26 @ entry $152.0058, W23-era re-entry at the reduced 26-sh size) was **closed 2026-07-01 pre_close** on a deterministic per-position stop breach: fresh Alpaca IEX bid 135.89 / ask 135.93 — both below the 136.84 stop, ~0.69% under, tight ~0.03% clean spread. Realized −$419.01 (−10.6%). Full gate chain APPROVED; `decisions/2026-07-01/1534_XOM.json`.
- **Sizing observation (confirms W22/W23 pattern):** XOM's paper-period closes are now 0-for-2, both energy-beta-driven — but the *mechanism* differed by size. At 97 sh (2026-05-29) a −9.35% move liquidated via the **daily-loss halt** (−$1,461). At 26 sh (this week) a larger −10.6% move stayed **under** the $500 daily-loss cap (−$419.01) and was closed by the ordinary **per-position stop** instead. The reduced-size re-entry (per `strategy_lessons/2026-w23.md` §Lesson 3) kept a comparable adverse XOM move from tripping the system-level halt — descriptive confirmation of the sizing hypothesis at its first stop-out.
- On 06-30 EOD the stop breach could NOT be actioned: no trustworthy close (quote ~37 min stale, ~10% degraded spread; bars 13d stale). It was correctly HELD via NO_TRADE and resolved only on 07-01's fresh tight-spread print — a clean illustration that the staleness rule (#5) deferred an exit until actionable data existed, rather than acting on a dislocated tick.
- Post-exit reclaim question (XOM >= 136.84 into the 07-01 close) is **UNRESOLVED** on stale data; deferred to >= 2026-07-06.
- Re-entry blocked 07-02 / 07-03 (NO_TRADE): a fresh ENTRY re-fired on stale 06-15/06-16 bars one session after the stop-out — re-entering the same name on a stale, pre-stop-out rank is precisely the whipsaw the staleness rule guards against.
- Next earnings 2026-07-31 — outside any window this period.
